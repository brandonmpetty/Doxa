 ...
 */
const Wasm = require('../../Bindings/WebAssembly/dist/doxaWasm.js');
...
class Image {
	...
	data = function() {
		return new Uint8ClampedArray(Wasm.HEAPU8.buffer, this.heapPtr, this.size);
	};
...
class Doxa {
	...
	static initialize = async function() {
		return new Promise((resolve) => {
			Wasm.onRuntimeInitialized = async _ => {
				...
				const algorithms = {}
				for (const key in Wasm.Binarization.Algorithms) {
					const alg = Wasm.Binarization.Algorithms[key];
	...
	static toBinary = function(algorithm, imageIn, parameters, imageOut /* optional */) {
		...
		if (!imageOut) {
			imageOut = new Image(imageIn.width, imageIn.height);
		}
		
		const paramString = JSON.stringify(parameters);
		const binarization = new Wasm.Binarization(algorithm);
