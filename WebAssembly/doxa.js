(function (Doxa, Wasm) {
	Doxa.Algorithm = [
		...
		{ enum: 4, name: 'N.I.C.K.', 	params: { window: 75, k:-0.20 },
		...
		{ enum: 5, name: 'Su', 			params: { window: 75, k: 0.20 },
		...
		{ enum: 6, name: 'T.R. Singh', 	params: { window: 75, k: 0.20 },
		...
		{ enum: 7, name: 'ISauvola', 	params: { window: 75, k: 0.20 },
		...
		{ enum: 8, name: 'Wan', 		params: { window: 75, k: 0.20 },
	...
	function to8BitImage(imageBuffer, imageData) {
		...
		for (var idx = 0; idx < size32; idx += 4) {
			...
			// TODO: Apply our standard grayscale conversion
	...
	Doxa.toBinary = function(image, parameters) {
		Wasm._ToBinary(image.data.byteOffset, 
			parameters && parameters.window || 0, 
